...
*/
	...
	bool is_https = (u.m_eScheme == SCHEME_HTTPS);
...
*/
	...
	if( is_https ) {
		...
		if(fl >= 0) { fl &= ~O_NONBLOCK; fcntl(sock, F_SETFL, fl); }
		SSL_CTX *ctx = SSL_CTX_new(TLS_client_method());
		ssl = SSL_new(ctx);
		if(ssl) {
			SSL_set_fd(ssl, sock);
			SSL_set_tlsext_host_name(ssl, host);
			if(SSL_connect(ssl) <= 0) { SSL_free(ssl); ssl = NULL; }
			else { SSL_CTX_free(ctx); }
		}
		if(!ssl) { if(ctx) SSL_CTX_free(ctx); close(sock); *nPSock = -1; if(url)free(url); if(requestBuf)free(requestBuf); return -1; }
	}

	if( is_https && ssl ) {
		ret = SSL_write(ssl, requestBuf, (int)strlen(requestBuf));
	} else {
	...
	if( ret == -1){
		...
		*nPSock  = -1;
		...
		if( (is_https&&ssl ? SSL_write(ssl,requestBuf,(int)strlen(requestBuf)) : write(sock,requestBuf,strlen(requestBuf))) == -1){
			//pthread_mutex_lock(&mutexMemory);
			if (url)
			{
				free(url); url=NULL;
	...
	ret = read_header(sock, headerBuf, ssl);
	//cout << "ret = " << ret << endl;
	if(ret < 0) { 
		close(sock); 
		*nPSock = -1;
		...
		if(selectRet == 0 && timeout < 0){
			...
			*nPSock = -1;
			...
			return -1;
                ...
                if(is_https&&ssl)ret=SSL_read(ssl,pageBuf+bytesRead,iPage.m_nContentLength);else ret=read(sock,pageBuf+bytesRead,iPage.m_nContentLength);
...
int CHttp::read_header(int sock, char *headerPtr, SSL *ssl)
{
	/* SSL path: use SSL_read directly */
	if (ssl) {
		int bytesRead = 0, newlines = 0, ret;
		while (newlines != 2 && bytesRead < HEADER_BUF_SIZE - 1) {
			ret = SSL_read(ssl, headerPtr, 1);
			if (ret <= 0) {
			...
			bytesRead++;
			if (*headerPtr == '\r') { headerPtr++; continue; }
			else if (*headerPtr == '\n') newlines++;
			else newlines = 0;
			headerPtr++;
		}
