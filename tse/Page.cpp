...
void CPage::GetStatusCode(string headerBuf)
{
	...
	const char *charIndex = strstr(headerBuf.c_str(), "http/");
	if (charIndex == NULL)
	{
		m_nStatusCode = -1;
		return;
...
void CPage::GetContentLength(string headerBuf)
{
	...
	const char *charIndex = strstr(headerBuf.c_str(), "content-length");
...
bool CPage::NormalizeUrl(string& strUrl)
{
	...
	if( CStrFun::FindCase(strUrl, "http://") == string::npos && CStrFun::FindCase(strUrl, "https://") == string::npos ) return false;

	// convert "http://e.pku.cn" to "http://e.pku.cn/"
	idx = strUrl.rfind('/');
	if( idx < (CStrFun::FindCase(strUrl, "https://") != string::npos ? 9 : 8) ) {
		strUrl = strUrl + "/";
		return true;
	}
	...
	if( CStrFun::FindCase(strUrl, "http://") != 0 && CStrFun::FindCase(strUrl, "https://") != 0 ) return false;
